...
mean_S1 = round(0.122*double(springer_options.audio_segmentation_Fs));
std_S1 = round(0.022*double(springer_options.audio_segmentation_Fs));
mean_S2 = round(0.094*double(springer_options.audio_segmentation_Fs));
std_S2 = round(0.022*double(springer_options.audio_segmentation_Fs));


mean_systole = double(round(double(systolic_time)*double(springer_options.audio_segmentation_Fs))) - double(mean_S1);
std_systole = (25.0/1000.0)*double(springer_options.audio_segmentation_Fs);


mean_diastole = ((60.0/double(heartrate)) - double(systolic_time) - 0.094)*double(springer_options.audio_segmentation_Fs);
std_diastole = 0.07*mean_diastole + (6.0/1000.0)*double(springer_options.audio_segmentation_Fs);
...
min_S1 = (mean_S1 - 3.0*(std_S1));
if(min_S1<(springer_options.audio_segmentation_Fs/50.0))
    min_S1 = (springer_options.audio_segmentation_Fs/50.0);
end

min_S2 = (mean_S2 - 3*(std_S2));
if(min_S2<(springer_options.audio_segmentation_Fs/50.0))
    min_S2 = (springer_options.audio_segmentation_Fs/50.0);
end
max_S1 = (mean_S1 + 3.0*(std_S1));
max_S2 = (mean_S2 + 3.0*(std_S2));
