...
class TickScale(ttk.Frame):
    ...
    def __init__(self, master=None, **kwargs):
        ...
        :param tickpos: "w" or "e" (vertical scale), "n" or "s" (horizontal scale):
        if tickinterval is not 0, position of the ticks
        :type tickpos: str
        ...
        self.text_label = kwargs.pop('label', '')
        ...
        if self._tickinterval == 0 and self._resolution == 0:
            ...
            if 0 <= self._digits < d:
                ...
                if self._resolution > self._tickinterval:
                    self._tickinterval = self._resolution
                self.scale.configure(from_=round(self.scale.get('from'), self._digits),  # type: ignore
                                     to=round(self.scale.get('to'), self._digits))  # type: ignore
    ...
    def _display_value_horizontal(self, value):
        ...
        if self._showvalue:
            self.label.configure(text=self.text_label + ' = ' + self._formatter.format(float(value)))
            self.update_idletasks()
